...
export class AgentManager {
  ...
  /**
   * Map every running agent to its on-disk agent directory. Used by the stall
   * detector to read per-agent config.json overrides without re-discovering
   * the agent tree on every scan.
   */
  getAgentDirs(): Map<string, string> {
    const out = new Map<string, string>();
    for (const [name, entry] of this.agents) {
      out.set(name, entry.process.getAgentDir());
    }
    return out;
  }
